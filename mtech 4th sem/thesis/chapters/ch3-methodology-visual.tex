...
A shallow feed-forward network with non-linearity and dropout maps the attention-aggregated sequence representation to a calibrated confidence score for the manipulated class. We avoid formulae here and describe the head in practical terms; regularization (dropout) is applied between layers to improve generalization.
...
\begin{itemize}
    \item \textbf{Frame Sampling:} Videos are sampled uniformly (e.g., 10 fps) to a fixed-length sequence of 16 frames.
    \item \textbf{Face Cropping:} Face regions are cropped to focus on relevant content (no specific detector named to keep the description generic and implementation-agnostic).
    \item \textbf{Resize \& Normalize:} Crops resized to 112x112 or 224x224 and normalized using ImageNet statistics.
\end{itemize}
...
% Optimizer/schedule settings focused on stable training and generalization (no hyperparameter formulas here)
\subsection{Optimization}
The model is trained with BCE loss, using Adam optimizer with initial learning rate in the range 1e-4 to 3e-4, cosine decay or step scheduling, weight decay (~1e-4), and batch sizes set by memory constraints. Early stopping on validation AUC is employed.

% Concrete hyperparameters and system settings to enable reproducibility (hardware, sizes, dropout, etc.)
\subsection{Implementation Details}
\begin{itemize}
    \item \textbf{Backbone:} ResNet50 initialized from ImageNet-1K; early layers optionally frozen for stability.
    \item \textbf{Bi-GRU:} 2 layers, hidden size 256--512 per direction.
    \item \textbf{Attention:} 4--8 heads with model dimension aligned to Bi-GRU outputs; dropout 0.1--0.3.
    \item \textbf{Regularization:} Dropout and label smoothing (optional).
    \item \textbf{Hardware:} Training performed on modern GPUs; mixed precision enabled where available.
\end{itemize}
...
\node[node] (vid){Input Video};
\node[node, right=of vid] (sample){Frame Sampling};
\node[node, right=of sample] (resnet){ResNet50\\(per-frame)};
\node[node, right=of resnet] (bigru){Bi-GRU\\(2-layer, bi-dir)};
\node[node, right=of bigru] (mha){Multi-Head\\Attention};
\node[node, right=of mha] (cls){Classifier\\(Real/Fake)};
\draw[arrow] (vid) -- (sample);
\draw[arrow] (sample) -- (resnet);
\draw[arrow] (resnet) -- (bigru);
\draw[arrow] (bigru) -- (mha);
\draw[arrow] (mha) -- (cls);
\end{tikzpicture}
}%
\caption{Phase I Hybrid Architecture Pipeline}
