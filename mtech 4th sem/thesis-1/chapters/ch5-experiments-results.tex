...
\textbf{Phase I} uses an aggregated corpus drawn from FaceForensics++ \cite{rossler2019faceforensics}, Celeb-DF \cite{li2020celebdf}, and DFDC \cite{dolhansky2020dfdc}. Training/validation/test splits follow subject- or video-level partitions to avoid leakage across sets. Data augmentations are applied consistently across frames during training.

\textbf{Phase II} employs a consolidated multimodal dataset of 28{,}461 videos drawn from FakeAVCeleb and a stratified subset of AV-Deepfake1M++. Evaluations include overall performance, comparisons to unimodal and simple-fusion baselines, robustness under lossy compression, and demographic subgroup analysis where available.
...
The comparative analysis reported in the accompanying paper places the proposed hybrid model (AUC 0.925) against Deepfake-Eval-2024 video models (approximately 0.50 AUC) and representative Celeb-DF baselines including Xception-c23 (0.653), MesoInception4 (0.536), and FWA (0.569). The reported comparison attributes the improvement to the combined use of ResNet50, Bi-GRU, Multi-Head Attention, and aggregated training.
...
The multimodal study additionally evaluates the model under lossy compression and reports slower degradation than the unimodal baselines within the tested range.

\paragraph{Demographic Fairness}
On FakeAVCeleb, subgroup analysis across race and gender reports variance below approximately 2\% in accuracy across the available groups.

% What we learn by comparing Phase I (visual) and Phase II (multimodal) results
\section{Cross-Phase Comparison and Insights}
Taken together, the two studies show that temporal modeling is important in both settings. Phase~I uses Bi-GRU and temporal attention to improve video-only detection on the aggregated benchmark, while Phase~II extends temporal reasoning across audio and video and reports gains over unimodal and simple-fusion baselines.
...
\begin{itemize}
    ...
    \item \textbf{High-Quality Synchronization:} Highly optimized lip-sync and TTS pipelines can reduce detectable audio–visual mismatches.
    \item \textbf{Computational Cost:} The hybrid and multimodal models remain more computationally demanding than lightweight single-stream baselines.
\end{itemize}
...
\section{Summary}
Phase~I validates the reported effectiveness of hybrid spatial–temporal modeling for video-only detection. Phase~II demonstrates that explicitly modeling audio–visual synchronicity via cross-modal attention and bidirectional temporal modeling improves multimodal detection relative to the reported unimodal and simple-fusion baselines. The next chapter discusses the broader implications and limitations of these findings.