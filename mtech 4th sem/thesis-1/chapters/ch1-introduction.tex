...
\begin{enumerate}[leftmargin=*,label=\arabic*.]
    ...
    \item Develop preprocessing and augmentation pipelines for the selected visual and multimodal benchmarks.
    \item Establish evaluation protocols that report Accuracy, AUC-ROC, F1-score, Precision, Recall, baseline comparisons, and ablations, with compression robustness and fairness analysis where reported in the accompanying studies.
    \item Include subgroup analysis where dataset annotations permit.
\end{enumerate}

\section{Contributions}
This thesis makes the following contributions:
\begin{enumerate}[leftmargin=*,label=\arabic*.]
    \item \textbf{Hybrid video-only detection:} An architecture that combines a ResNet50 backbone for spatial encoding with Bidirectional GRUs for temporal dynamics and Multi-Head Attention for sequence focus. On the aggregated benchmark of FaceForensics++, Celeb-DF, and DFDC, the model reports 0.9266 Accuracy and 0.925 AUC.
    ...
    \item \textbf{Comparative evaluation:} Baseline comparisons and ablation studies are reported for both studies, with the multimodal setting additionally including lossy-compression robustness and demographic subgroup assessment.
\end{enumerate}
